...
完全背包与01背包的区别，01背包，每个物品只能用一次，而完全背包每个物品可以用无限次。

代码的区别就是把01背包的内循环遍历方式改成从小到大遍历即可。
...
/*
完全背包推导过程
max括号里的内容分别是第i个物品放0个，放1个，放2个,……,放k个
1.f[i][j]=max(f[i-1][j],f[i-1][j-v]+w,f[i-1][j-2*v]+2*w,...,f[i-1][j-k*v]+k*w)
我们把j替换成j-v
2.f[i][j-v]=max(f[i-1][j-v],f[i-1][j-2v]+2W,f[i-1][j-3v]+3W,...,f[i-1][j-(k+1)v]+(k+1)w)
我们用2式代替1式中max中除去第一项的所有项,会少一个w，所以替换的时候要多加一个w
所以
...
每个物品的个数是有限的，相当于01背包的扩展

二进制优化版本，当物品*物品个数很大的时候，我们可以考虑使用二进制优化。即把一个数字最少拆成几个整数，这几个整数可以表示0~该数之间所有的数



### 分组背包

每个组的物品只能选一种，多重背包其实是分组背包的一种特例。这里的做法是把每一组的每一个物品都判断一遍。
